...
const initialState = {
	...
	mode: '',
	detailKontak: {
		element: 0, message: 'OK', page: '', result: [{
			id: '',
			name: '',
			address: '',
			phone: '',
...
const Reducers = (state = initialState, action) => {
	switch (action.type) {
		case 'addData':
			return{...state}
		break;
		case 'toogleAddViewBatal':
			return{...state, keyToogle: action.keyToogle}
		break;

		case 'editString':
			console.log("Aku berada di Reducer");
			console.log(action);
			return { ...state, testString: action.data.dataBaru }
			break;

		case 'GetDetailsContact':
			console.log("action : ", action.payload)
			return { ...state, kontak: action.payload }
			break;
...
class Home extends React.Component {
	render() {
		var Page = [];
		const pathrouter = (this.props.match.url).substring(1); //ngambil url
