	...
	if (props.state.detailKontak.result.length>0){
		var listViewItems=props.state.detailKontak.result.map((items,i)=>{
			...
			return (<tr key={i}>
						...
						<td>{items.phone}</td>
						<td>{items.email}</td>
						<td><button onClick={()=>props.handleEdit(items)} style={{width:'40px'}} className="btn btn-default text-center"><i className="fa fa-edit" ></i></button> 
						<button onClick={()=>props.handleDelete(items.id)} style={{width:'40px'}} className="btn btn-danger"><i className="fa fa-trash" ></i></button></td>
					</tr>
			)
		});
		return (
